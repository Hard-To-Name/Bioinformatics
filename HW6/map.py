...
def calculate(graph, queue):
    while not len(queue) == 0:
        for k, v in graph.adj[queue[0]].items():
            if graph.nodes[k].get('cost', -1) == -1:
                queue.append(k)
            else:
                if graph.nodes[queue[0]].get('cost', -1) == -1:
                    graph.nodes[queue[0]]['cost'] = graph.nodes[k]['cost'] + v['weight']
                    graph.nodes[queue[0]]['parent'] = k
                else:
                    graph.nodes[queue[0]]['cost'] = min(graph.nodes[queue[0]]['cost'], graph.nodes[k]['cost'] + v['weight'])
                    graph.nodes[queue[0]]['parent'] = k
        queue.pop(0)

def track_path(graph, path):
    last = len(path) - 1
    if path[last] == 'Start': return
    path.append(graph.nodes[path[last]]['parent'])
    track_path(graph, path)

mygraph.nodes['Start']['cost'] = 0
queue = [k for k, v in mygraph.adj['Start'].items()]
calculate(mygraph, queue)
path = ['End']
track_path(mygraph, path)
path = path[::-1]
print("Path:", path)
print("Cost:", mygraph.nodes['End']['cost'])
